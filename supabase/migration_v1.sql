-- ============================================
-- CherryPick Engine — Migration V1
-- Execute in Supabase SQL Editor in one shot
-- ============================================

-- 1. batch_jobs (no FK dependencies)
...
-- 3. jobs (references personas, batch_jobs — assets FK added later)
CREATE TABLE jobs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    trend_asset_id UUID,
    image_asset_id UUID,
    video_raw_asset_id UUID,
    video_final_asset_id UUID,
    celery_task_id TEXT,
    error_message TEXT,
...
-- 5. Add FK from jobs to assets
ALTER TABLE jobs ADD CONSTRAINT fk_jobs_trend_asset FOREIGN KEY (trend_asset_id) REFERENCES assets(id);
ALTER TABLE jobs ADD CONSTRAINT fk_jobs_image_asset FOREIGN KEY (image_asset_id) REFERENCES assets(id);
ALTER TABLE jobs ADD CONSTRAINT fk_jobs_video_raw_asset FOREIGN KEY (video_raw_asset_id) REFERENCES assets(id);
ALTER TABLE jobs ADD CONSTRAINT fk_jobs_video_final_asset FOREIGN KEY (video_final_asset_id) REFERENCES assets(id);

-- 6. Add FK from batch_jobs to personas
ALTER TABLE batch_jobs ADD CONSTRAINT fk_batch_jobs_persona FOREIGN KEY (persona_id) REFERENCES personas(id);
